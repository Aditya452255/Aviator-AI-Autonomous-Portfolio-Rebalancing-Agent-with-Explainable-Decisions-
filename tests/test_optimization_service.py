"""Unit tests for OptimizationService orchestration and data exports."""
...
def test_optimization_service_full_cycle(tmp_path: Path) -> None:
    """Test full Phase 3 optimization service workflow."""
    ...
    _, queue, _, _ = mon_service.run_monitoring_cycle(
    ...
    results, analytics = opt_service.run_optimization_cycle(
        portfolios=portfolios,
    ...
    assert len(results) == len(queue)
    assert "optimization_statistics" in analytics

    # Check exported files
    assert (tmp_path / "optimized_portfolios.parquet").exists()
    assert (tmp_path / "trade_list.parquet").exists()
    assert (tmp_path / "execution_plan.parquet").exists()
    assert (tmp_path / "optimization_summary.json").exists()
    assert (tmp_path / "cost_analysis.csv").exists()
    assert (tmp_path / "tax_analysis.csv").exists()
